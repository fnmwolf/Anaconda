...
LPCWSTR SELECTOR_TITLES[] = {
    L"Save as Anaconda (Windows)",
    L"Save as Anaconda (Mac OS X)",
    L"Save as Anaconda (Linux)"
};
...
BOOL WINAPI Build (LPCWSTR pTargetPathname, LPCWSTR pCCNPathname, int idx, DWORD buildFlags)
{
    ...
    swprintf(directory, L"%s%s", tempdirectory, L"\\Data\\Runtime\\Anaconda\\");
    ...
    else
    {
        ...
        // Display message.
        MessageBox(NULL, Error, "Couldn't open build process", MB_OK|MB_ICONWARNING );
