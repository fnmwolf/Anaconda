...
b2PolygonShape::b2PolygonShape(const b2ShapeDef* def, b2Settings* set)
	 : b2Shape(def, set)
{
	...
	b2Assert(3 <= m_vertexCount);

	m_vertices = (b2Vec2*)((char*)this + sizeof(b2PolygonShape));
	m_normals = (b2Vec2*)((char*)this + sizeof(b2PolygonShape) + m_vertexCount * sizeof(b2Vec2));
	m_coreVertices = (b2Vec2*)((char*)this + sizeof(b2PolygonShape) + m_vertexCount * sizeof(b2Vec2) * 2);
	...
	for (int32 i = 0; i < m_vertexCount; ++i)
	{
		...
		b2Assert2(edge.LengthSquared() > B2_FLT_EPSILON * B2_FLT_EPSILON,"Invalid Polygon Created! Polygon Edge is too Short!");
	...
	for (int32 i = 0; i < m_vertexCount; ++i)
	{
		for (int32 j = 0; j < m_vertexCount; ++j)
		{
			...
			b2Assert2(s < -(settings->b2_linearSlop),"Invalid Polygon Created! Polygon isn't Convex or too Thin!");
		}
	}
	...
	for (int32 i = 1; i < m_vertexCount; ++i)
	{
		...
		b2Assert2(angle > settings->b2_angularSlop,"Invalid Polygon Created! Polygon isn't Clockwise!");
	}
#endif
	...
	for (int32 i = 0; i < m_vertexCount; ++i)
	{
		...
		b2Vec2 d;
		d.x = b2Dot(n1, v) - settings->b2_toiSlop;
		d.y = b2Dot(n2, v) - settings->b2_toiSlop;

		// Shifting the edge inward by b2_toiSlop should
		// not cause the plane to pass the centroid.

		// Your shape has a radius/extent less than b2_toiSlop.
		b2Assert2(d.x >= 0.0f && d.y >= 0.0f, "Invalid Polygon Created! Core Shape got too Small!");
		b2Mat22 A;
												...
												b2Vec2* c) const
{
	...
	float32 *depths = new float32[m_vertexCount];
	int32 diveCount = 0;
	int32 intoIndex = -1;
	int32 outoIndex = -1;
