...
struct b2EdgeChainDef : public b2ShapeDef
{
	...
	b2EdgeChainDef(int num)
	{
		type = e_edgeShape;
		vertexCount = 0;
		isALoop = true;
		vertexMax = num;
		vertices = new b2Vec2[num];
	}
	~b2EdgeChainDef()
	{
	...
	int defSize(){return sizeof(b2EdgeChainDef);}
...
private:
	...
	b2EdgeShape(const b2Vec2& v1, const b2Vec2& v2, const b2ShapeDef* def, b2Settings* set);
