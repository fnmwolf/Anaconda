...
void b2PairManager::Initialize(b2BroadPhase* broadPhase, b2PairCallback* callback, b2Settings* set)
{
	settings = set;

	b2Assert(b2IsPowerOfTwo(set->b2_tableCapacity) == true);
	b2Assert(set->b2_tableCapacity >= set->b2_maxPairs);

	m_pairBuffer = new b2BufferedPair[set->b2_maxPairs];
	m_pairs = new b2Pair[set->b2_maxPairs];
	m_hashTable = new uint16[set->b2_tableCapacity];

	for (int32 i = 0; i < set->b2_tableCapacity; ++i)
	{
		m_hashTable[i] = b2_nullPair;
	}
	m_freePair = 0;
	for (int32 i = 0; i < set->b2_maxPairs; ++i)
	{
	...
	m_pairs[set->b2_maxPairs-1].next = b2_nullPair;
	m_pairCount = 0;
	m_pairBufferCount = 0;
...
b2Pair* b2PairManager::Find(int32 proxyId1, int32 proxyId2, uint32 hash)
{
	...
	b2Assert(index < settings->b2_maxPairs);
...
b2Pair* b2PairManager::Find(int32 proxyId1, int32 proxyId2)
{
	...
	int32 hash = Hash(proxyId1, proxyId2) & settings->b2_tableMask;
...
b2Pair* b2PairManager::AddPair(int32 proxyId1, int32 proxyId2)
{
	...
	b2Assert(m_pairCount < settings->b2_maxPairs && m_freePair != b2_nullPair);
...
void b2PairManager::AddBufferedPair(int32 id1, int32 id2)
{
	...
	b2Assert(m_pairBufferCount < settings->b2_maxPairs);
...
void b2PairManager::Commit()
{
	...
	for (int32 i = 0; i < m_pairBufferCount; ++i)
	{
		...
		b2Assert(pair->proxyId1 < settings->b2_maxProxies && pair->proxyId2 < settings->b2_maxProxies);
...
void b2PairManager::ValidateBuffer()
{
	...
	for (int32 i = 0; i < m_pairBufferCount; ++i)
	{
		...
		b2Assert(pair->proxyId1 < settings->b2_maxProxies);
		b2Assert(pair->proxyId2 < settings->b2_maxProxies);
