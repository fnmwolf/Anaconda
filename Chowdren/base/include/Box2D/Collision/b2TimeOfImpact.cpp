					   ...
					   b2Settings* set)
{
	...
	for(;;)
	{
		...
		if (iter == 0)
		{
			...
			if (distance > 2.0f * set->b2_toiSlop)
			{
				targetDistance = 1.5f * set->b2_toiSlop;
			}
			else
			{
				targetDistance = b2Max(0.05f * set->b2_toiSlop, distance - 0.5f * set->b2_toiSlop);
			}
		}

		if (distance - targetDistance < 0.05f * set->b2_toiSlop || iter == k_maxIterations)
		{
			break;
		}
