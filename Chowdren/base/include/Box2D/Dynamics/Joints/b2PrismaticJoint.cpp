...
b2PrismaticJoint::b2PrismaticJoint(const b2PrismaticJointDef* def, b2Settings* set)
: b2Joint(def, set)
{
...
void b2PrismaticJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	if (m_enableLimit)
	{
		...
		if (b2Abs(m_upperTranslation - m_lowerTranslation) < 2.0f * settings->b2_linearSlop)
		{
...
bool b2PrismaticJoint::SolvePositionConstraints(float32 baumgarte)
{
	...
	if (m_enableLimit)
	{
		...
		if (b2Abs(m_upperTranslation - m_lowerTranslation) < 2.0f * settings->b2_linearSlop)
		{
			...
			C2 = b2Clamp(translation, -(settings->b2_maxLinearCorrection), settings->b2_maxLinearCorrection);
		...
		else if (translation <= m_lowerTranslation)
		{
			...
			C2 = b2Clamp(translation - m_lowerTranslation + settings->b2_linearSlop, -(settings->b2_maxLinearCorrection), 0.0f);
		...
		{
			// Prevent large linear corrections and allow some slop.
			C2 = b2Clamp(translation - m_upperTranslation - settings->b2_linearSlop, 0.0f, settings->b2_maxLinearCorrection);
