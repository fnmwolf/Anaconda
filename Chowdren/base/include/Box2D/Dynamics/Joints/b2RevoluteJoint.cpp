...
b2RevoluteJoint::b2RevoluteJoint(const b2RevoluteJointDef* def, b2Settings* set)
: b2Joint(def, set)
{
...
void b2RevoluteJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	if (m_enableLimit)
	{
		...
		if (b2Abs(m_upperAngle - m_lowerAngle) < 2.0f * settings->b2_angularSlop)
		{
...
bool b2RevoluteJoint::SolvePositionConstraints(float32 baumgarte)
{
	...
	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		...
		if (m_limitState == e_equalLimits)
		{
			...
			float32 C = b2Clamp(angle - m_lowerAngle, -(settings->b2_maxAngularCorrection), settings->b2_maxAngularCorrection);
		...
		else if (m_limitState == e_atLowerLimit)
		{
			...
			C = b2Clamp(C + settings->b2_angularSlop, -(settings->b2_maxAngularCorrection), 0.0f);
			limitImpulse = -m_motorMass * C;
		}
		else if (m_limitState == e_atUpperLimit)
		{
			...
			// Prevent large angular corrections and allow some slop.
			C = b2Clamp(C - settings->b2_angularSlop, 0.0f, settings->b2_maxAngularCorrection);
			limitImpulse = -m_motorMass * C;
		}
	...
	// Solve point-to-point constraint.
	{
		...
		const float32 k_allowedStretch = 10.0f * settings->b2_linearSlop;
