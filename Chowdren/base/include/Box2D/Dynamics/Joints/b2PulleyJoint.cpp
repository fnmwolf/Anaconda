				...
				float32 r, float32 minLength)
{
	...
	maxLength1 = C - ratio * minLength;
	maxLength2 = (C - minLength) / ratio;
}

b2PulleyJoint::b2PulleyJoint(const b2PulleyJointDef* def, b2Settings* set)
: b2Joint(def, set)
{
	...
	m_maxLength1 = b2Min(def->maxLength1, m_constant - m_ratio * def->minLength);
	m_maxLength2 = b2Min(def->maxLength2, (m_constant - def->minLength) / m_ratio);
...
bool b2PulleyJoint::SolvePositionConstraints(float32 baumgarte)
{
	...
	if (m_state == e_atUpperLimit)
	{
		...
		C = b2Clamp(C + settings->b2_linearSlop, -(settings->b2_maxLinearCorrection), 0.0f);
