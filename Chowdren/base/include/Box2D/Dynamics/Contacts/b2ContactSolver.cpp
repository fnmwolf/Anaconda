...
b2ContactSolver::b2ContactSolver(const b2TimeStep& step, b2Contact** contacts, int32 contactCount, b2StackAllocator* allocator, b2Settings* set)
{
	settings = set;
	m_step = step;
...
bool b2ContactSolver::SolvePositionConstraints(float32 baumgarte)
{
	...
	for (int32 i = 0; i < m_constraintCount; ++i)
	{
		...
		for (int32 j = 0; j < c->pointCount; ++j)
		{
			...
			float32 C = baumgarte * b2Clamp(separation + settings->b2_linearSlop, -(settings->b2_maxLinearCorrection), 0.0f);
