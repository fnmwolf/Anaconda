...
void b2Island::Solve(const b2TimeStep& step, const b2Vec2& gravity, bool allowSleep)
{
	...
	for (int32 i = 0; i < m_bodyCount; ++i)
	{
		...
		if (b2Dot(b->m_linearVelocity, b->m_linearVelocity) > settings->b2_maxLinearVelocitySquared)
		{
			b->m_linearVelocity.Normalize();
			b->m_linearVelocity *= settings->b2_maxLinearVelocity;
		}
		if (b->m_angularVelocity * b->m_angularVelocity > settings->b2_maxAngularVelocitySquared)
		{
			if (b->m_angularVelocity < 0.0f)
			{
				b->m_angularVelocity = -(settings->b2_maxAngularVelocity);
			}
			else
			{
				b->m_angularVelocity = settings->b2_maxAngularVelocity;
	...
	b2ContactSolver contactSolver(step, m_contacts, m_contactCount, m_allocator, settings);
	...
	{
		bool contactsOkay = contactSolver.SolvePositionConstraints(settings->b2_contactBaumgarte);

		bool jointsOkay = true;
		for (int32 i = 0; i < m_jointCount; ++i)
		{
			bool jointOkay = m_joints[i]->SolvePositionConstraints(settings->b2_contactBaumgarte);
	...
	if (allowSleep)
	{
		...
		const float32 linTolSqr = settings->b2_linearSleepTolerance * settings->b2_linearSleepTolerance;
		const float32 angTolSqr = settings->b2_angularSleepTolerance * settings->b2_angularSleepTolerance;
#endif
...
{
	b2ContactSolver contactSolver(subStep, m_contacts, m_contactCount, m_allocator, settings);
...
void b2Island::Report(b2ContactConstraint* constraints)
{
	...
	for (int32 i = 0; i < m_contactCount; ++i)
	{
		...
		if ( !(c->GetShape1()->IsSensor() || c->GetShape2()->IsSensor()) )
		{
			...
			for (int32 j = 0; j < manifoldCount; ++j)
			{
				...
				for (int32 k = 0; k < manifold->pointCount; ++k)
				{
					...
					m_listener->ContactPointResponse(c, &cr);
				}
			}
		}
	}
