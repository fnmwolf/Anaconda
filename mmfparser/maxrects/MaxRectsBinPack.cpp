...
bool MaxRectsBinPack::Insert(const std::vector<RectSize> &rects, std::vector<Rect> &dst, std::vector<int> &dstidx, FreeRectChoiceHeuristic method)
{
	dst.clear();
	dstidx.clear();

	dst.reserve(rects.size());
	dstidx.reserve(rects.size());
	...
	while(idx.size() > 0)
	{
		...
		for(size_t i = 0; i < idx.size(); ++i)
		{
			int score1;
			int score2;
			Rect newNode = ScoreRect(rects[idx[i]].width, rects[idx[i]].height, method, score1, score2);
		...
		dstidx.push_back(idx[bestRectIndex]);
